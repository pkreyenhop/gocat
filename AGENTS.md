...
- Naming: directions as `DirFwd`/`DirBack`, caret/selection fields as `Caret`, `Sel`, `Leap`. Use `[]rune` for buffer text to preserve Unicode indexing. Buffers are tracked via `app.buffers`/`bufIdx`; keep UI-facing shortcuts (`Ctrl+B` new buffer, `Tab`/`Shift+Tab` cycle, `Ctrl+O` picker `Ctrl+L` load, `Ctrl+W` save, `Ctrl+Shift+S` save dirty buffers, `Ctrl+Q`/`Ctrl+Shift+Q` close/quit, `Ctrl+/` comment toggle, `Ctrl+Shift+/` help buffer, Delete word (case-insensitive leap), Shift+Delete line) mapped in `handleEvent`. Double-space indents by inserting a tab at line start.
