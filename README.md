...
| New buffer / cycle buffers | Ctrl+B / Ctrl+Tab / Ctrl+Shift+Tab |
...
- Creates an `editor.Editor`, injects an SDL-backed clipboard, and enters an event loop that maps Cmd+typing into Leap (case-insensitive), dual-Cmd selection, Ctrl+Cmd “Leap Again,” Ctrl+C/X/V clipboard, `Ctrl+B` new `<untitled>` buffer, `Ctrl+Tab`/`Ctrl+Shift+Tab` buffer cycle, `Ctrl+O` file picker, `Ctrl+L` load selected file, `Ctrl+W` save (prompts for filename when unnamed via the input line), `Ctrl+F` save+fmt/fix+reload, `Ctrl+Q` quit current buffer, and plain text/caret edits when Leap is inactive.
